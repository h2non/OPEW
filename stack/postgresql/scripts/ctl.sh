...
HOSTNAME=`hostname`
POSTGRESQL_PIDFILE=/opt/opew/stack/postgresql/data/postmaster.pid

POSTGRESQL_START="/opt/opew/stack/postgresql/bin/pg_ctl start -w -D /opt/opew/stack/postgresql/data"
POSTGRESQL_STOP="/opt/opew/stack/postgresql/bin/pg_ctl stop -D /opt/opew/stack/postgresql/data"
...
start_postgresql() {
	...
	su opew-postgres -c "$POSTGRESQL_START"
    else
	$POSTGRESQL_START 
    fi
    sleep 3
...
stop_postgresql() {
    ...
    if [ `id|sed -e s/uid=//g -e s/\(.*//g` -eq 0 ]; then
        su opew-postgres -c "$POSTGRESQL_STOP"
    else
        $POSTGRESQL_STOP
    fi
    sleep 5
